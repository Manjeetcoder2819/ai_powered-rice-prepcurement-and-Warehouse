...
def _migrate_sqlite_schema(conn):
    ...
    required_columns = {
        "batches": {
            "bag_media_url": "VARCHAR DEFAULT ''",
            "bag_media_type": "VARCHAR DEFAULT ''",
            "damage_media_url": "VARCHAR DEFAULT ''",
            "damage_class": "VARCHAR DEFAULT ''",
            "damage_confidence": "FLOAT DEFAULT 0.0",
            "detection_source": "VARCHAR DEFAULT 'simulated'",
        },
        "vehicles": {
            "last_gate_image_url": "VARCHAR DEFAULT ''",
            "last_plate_detected": "VARCHAR DEFAULT ''",
            "last_anpr_confidence": "FLOAT DEFAULT 0.0",
            "last_gate_decision": "VARCHAR DEFAULT ''",
        },
    }
...
async def seed_database():
    ...
    async def seed_variety_prices(db: AsyncSession) -> None:
        prices = {
            "Sona Masoori": 33.0,
            "IR-64": 28.5,
            "Basmati": 39.0,
        ...
        existing = await db.execute(select(VarietyPriceModel))
        by_variety = {record.variety: record for record in existing.scalars().all()}

        for variety, price in prices.items():
            record = by_variety.get(variety)
            if record:
                record.price_per_kg = price
            else:
                db.add(VarietyPriceModel(variety=variety, price_per_kg=price))

    async def add_missing_by_key(db: AsyncSession, model, records: list, key: str) -> None:
        values = [getattr(record, key) for record in records]
        with db.no_autoflush:
            existing = await db.execute(
                select(model).where(getattr(model, key).in_(values))
            )
        existing_values = {getattr(record, key) for record in existing.scalars().all()}
        db.add_all(
            [record for record in records if getattr(record, key) not in existing_values]
        )

    async def add_seed_rows_if_empty(db: AsyncSession, model, records: list) -> None:
        with db.no_autoflush:
            existing = await db.execute(select(model).limit(1))
    ...
    async with AsyncSessionLocal() as db:
        ...
        await add_missing_by_key(db, FarmerModel, farmers_data, "token")
        ...
        await add_missing_by_key(db, BatchModel, batches_data, "batch_id")
        ...
        await add_missing_by_key(db, StockModel, stock_data, "variety")
        ...
        await add_missing_by_key(db, ChecklistModel, checklist_data, "text")
        ...
        await add_missing_by_key(db, ZoneModel, zones_data, "zone_id")
        ...
        await add_seed_rows_if_empty(db, LedgerModel, ledger_data)
        ...
        await add_missing_by_key(db, VehicleModel, vehicles_data, "vehicle_id")
        ...
        await add_seed_rows_if_empty(db, SMSLogModel, sms_data)
