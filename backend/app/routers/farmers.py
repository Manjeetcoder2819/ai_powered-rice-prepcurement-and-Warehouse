...
async def create_farmer(data: FarmerCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Generate token from existing token values instead of row count, so
    # partial deletions cannot reuse a live token.
    result = await db.execute(select(FarmerModel.token))
    token_numbers = []
    for existing_token in result.scalars().all():
        match = re.fullmatch(r"F(\d+)", existing_token or "")
        if match:
            token_numbers.append(int(match.group(1)))
    token = f"F{(max(token_numbers, default=23) + 1):03d}"
