...
async def get_kpis(db: AsyncSession = Depends(get_db)):
    ...
    # Calculate increments dynamically from database batches beyond initial seeded ones
    db_total_bags = bags_q.scalar() or 0
    db_damaged = dmg_q.scalar() or 0
    
    new_bags_scanned = max(0, db_total_bags - 205) # 205 is seeded bags count (B-0082 + B-0081)
    new_damaged_scanned = max(0, db_damaged - 9)    # 9 is seeded damaged count (6 + 3)
    
    total_bags = 2543 + new_bags_scanned
    damaged = 37 + new_damaged_scanned
    
    # Dynamic calculations based on new additions
    rice_procured_kg = 128450.0 + (new_bags_scanned * 50.0)
    bags_today = 156 + new_bags_scanned
    
    # Count new alerts dynamically (batches with defects)
    alerts_db_q = await db.execute(
        select(func.count(BatchModel.id)).where((BatchModel.damage_pct >= 5.0) | (BatchModel.wet > 0))
    )
    db_alerts = alerts_db_q.scalar() or 0
    new_alerts = max(0, db_alerts - 2) # B-0082 and B-0081 both had defects
    alerts_today = 12 + new_alerts
    ...
    return {
        ...
        "farmers_delta":         -8,
        "rice_procured_kg":      rice_procured_kg,
        "rice_delta_pct":        12.5,
