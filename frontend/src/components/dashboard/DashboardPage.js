...
function QualityTrendChart({ batches = [] }) {
  ...
  recent.forEach((b, idx) => {
    ...
    // Safely map percentages to y-range [12, 82] (leaving 18px at the bottom for labels and 12px at the top)
    const yGood = 82 - (gPct / 100) * 70;
    
    // Scale defect rates by 3x to be visually visible, but clamp at 100% (y = 12) so it never overflows
    const yWet = 82 - (Math.min(100, wPct * 3) / 100) * 70;
    const yDamaged = 82 - (Math.min(100, dPct * 3) / 100) * 70;
  ...
  // Use 82 as the baseline for filled area polygons
  const fillGood = dGood ? `${dGood} L ${pointsGood[pointsGood.length-1].x} 82 L ${pointsGood[0].x} 82 Z` : '';
  const fillWet = dWet ? `${dWet} L ${pointsWet[pointsWet.length-1].x} 82 L ${pointsWet[0].x} 82 Z` : '';
  const fillDamaged = dDamaged ? `${dDamaged} L ${pointsDamaged[pointsDamaged.length-1].x} 82 L ${pointsDamaged[0].x} 82 Z` : '';
  ...
  return (
    <div style={{ background: '#ffffff', borderRadius: 14, padding: 16, border: '1px solid #e5e7eb', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
      ...
      <svg width="100%" height="95" viewBox="0 0 270 100" style={{ overflow: 'hidden' }}>
        <defs>
          ...
          <table style={tableStyle}>
            ...
            <tbody>
              {(queueFarmers.length
                ...
                return (
                  <tr key={f.id ? `dfarm-${f.id}-${f.token || index}` : `dfarm-${index}`} style={{ borderBottom: '1px solid #f1f5f9' }}>
      ...
      <div style={mainGrid}>
        ...
        <div style={cardStyle}>
          ...
          <table style={tableStyle}>
            ...
            <tbody>
              {(vehicles.length ? vehicles : DEMO_VEHICLES).map((v, index) => (
                <tr key={v.id || v.vehicle_id ? `dveh-${v.id || v.vehicle_id}-${index}` : `dveh-${index}`}>
                  <td>{v.id}</td>
