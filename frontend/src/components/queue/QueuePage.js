...
export default function QueuePage() {
  ...
  const load = async (isInitial = false) => {
    try {
      ...
      console.log(error)
      // Only show demo data on the very first load when we have nothing
      // real yet. A later poll failing (e.g. a brief network hiccup)
      // must NOT wipe out real farmers already showing on screen.
      if (isInitial) {
        setFarmers(DEMO_FARMERS)
      } else {
        toast.error('Could not refresh queue — showing last known data')
      }
    }
  }

  useEffect(() => {
    const initialLoad = setTimeout(() => load(true), 0)
          ...
          <table style={tableStyle}>
            ...
            <tbody>
              {filtered.map((f, index) => {
                const s =
                  STATUS_STYLE[f.status] || STATUS_STYLE.waiting

                return (
                  <tr key={f.id ? `farmer-${f.id}-${f.token || index}` : `farmer-${index}`}>
                    <td>{index + 1}</td>
